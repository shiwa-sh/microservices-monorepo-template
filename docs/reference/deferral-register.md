...
| ClickHouse | funnel-query p95 above 2s against the declared rollup window, or the events table sustaining more than about 10M rows per month | **uncollected** — both are measurable against the analytics store, and nothing emits them until one runs | [ADR-0700](../adr/0700-analytics.md) |
...
| Registry mirroring | an upstream registry rate-limits or removes a pinned digest the cluster depends on | **alert** — `ImagePullsFailing`, on the kubelet pull-error metric the node agent now scrapes | [ADR-0105](../adr/0105-image-registry.md) |
...
**Four rows are alerted**, by `VolumeExceedsHalfNodeDisk`, `LoadGeneratorDroppingIterations` and `ImagePullsFailing` in `infra/observability/alerts/deferral-triggers.yaml`, and by `ActiveSeriesNearCeiling` in `cardinality.yaml`. A deferral whose trigger is alertable and unalerted is indistinguishable from a deferral with no trigger, so a row reaches this state only when a rule names the condition the ADR wrote down.

**One row is `uncollected`**: the analytics store that would answer the funnel-latency question is not deployed, so nothing emits the series a rule would read. Adding the rule without the series would produce a rule that can never fire, which is the same blindness with a green tick on it. The registry row left this state the way any row does — by something collecting its signal first, then a rule naming it.
