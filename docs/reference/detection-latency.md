...
| Backup restore is rehearsed quarterly | a backup that cannot be restored is discovered at the rehearsal, or at the restore | [ADR-0200](../adr/0200-cluster-topology.md) |
...
| A `marketing.*` event reaching the log store | review of the collector's routing connector ([ADR-0700](../adr/0700-analytics.md)) | **unbounded** | **unbounded** |

## Reading the table

**Three rows are unbounded, and they are the ones to change first.** An unbounded row is not a long detection time; it is the absence of detection, and the fault is found by its consequence rather than by a signal. Two of them — the missing Watchdog and the analytics routing split — are named in their own ADRs as gaps with no mechanism against them.

**The dominant term is not the alerting stack.** Rules evaluate in seconds and route in seconds. What sets nine of these rows is that the receiver reaches an inbox rather than a person, so the honest summary is: **this platform detects at the speed of someone looking.**
