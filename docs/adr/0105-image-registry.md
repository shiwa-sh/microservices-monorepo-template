...
- **Related:** [ADR-0000](0000-platform-foundations.md), [ADR-0102](0102-source-control-and-ci.md), [ADR-0103](0103-release-and-versioning.md), [ADR-0104](0104-supply-chain-security.md)
...
3. **Object storage as the backend** ([ADR-0200](0200-cluster-topology.md)), so registry durability is the bucket's durability and not a second backup problem.
4. **Availability under node churn.** Pulls must survive the control plane being busy.
5. **Configuration in the repository** (principle 1). Projects, quotas, and retention are files, not UI state.
...
| Harbor | registry, core, jobservice, portal, **its own Postgres and Redis**, plus a scanner | yes | partly — projects, robots, and retention are API and UI objects, reconciled only by a separate operator | Feature-complete and the reflexive answer. Six workloads and a second datastore for one concern is the purchase principle 2 exists to refuse |
| CNCF Distribution (`registry:2`) | one binary | yes | one config file | The reference implementation, and the thinnest. No authentication model beyond htpasswd or a token service, no scanning, no UI, no retention policy — each of which then becomes its own decision |
| Forgejo package registry | none — already running ([ADR-0102](0102-source-control-and-ci.md)) | partial | forge config | Free in component count and rejected anyway: it couples every pod start to forge availability, and puts the artefact store inside the system that builds it |
...
**Scanning is not the registry's job.** [ADR-0104](0104-supply-chain-security.md) puts vulnerability scanning in CI, where a finding blocks a merge. A registry-side scan reports on images that already shipped, which is a dashboard rather than a gate. zot can run a scanner; it stays off, and the concern stays in one place ([ADR-0000](0000-platform-foundations.md), principle 5).
...
- One binary on the floor instead of six workloads and a second datastore.
...
- Registry configuration is a committed file. Projects, quotas, and retention are never set through an API call or a UI. `(review-only)`
- Signatures, SBOMs, and provenance are OCI referrers on the image they describe ([ADR-0104](0104-supply-chain-security.md)). `(review-only)`
- Vulnerability scanning runs in CI, not in the registry. `(review-only)`
