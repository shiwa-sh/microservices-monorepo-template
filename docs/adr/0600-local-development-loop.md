...
**The full tier provides its own storage.** Talos ships no provisioner, so the tier runs the same `local-path` chart a deployed environment does and gets the same default StorageClass. The inner loop is the exception, because kind brings its own — enabling the chart there would put two provisioners on one default annotation. Without it there is no StorageClass at all, and every stateful component sits Pending on a claim that names the claim rather than the missing provisioner.
...
- **The full tier costs laptop RAM**, and the number is 12 GB: three nodes at 4 GB, because the control plane alone runs etcd, the API server, Cilium and Argo CD's controller. Under-provisioning it does not degrade gracefully — the API server starts timing out, and what a developer sees is `TLS handshake timeout` from kubectl rather than anything about memory. Per-service declarations exist so day-to-day work runs only its slice on the smaller tier.
