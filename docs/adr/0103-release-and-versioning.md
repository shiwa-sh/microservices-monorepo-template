...
- The repository has one release version, CalVer `vYYYY.0M.MICRO`. There are no per-component version lines or prefixed tag namespaces.
...
- Anything extracted or published for external consumers to pin adopts SemVer with its own tag at that point.
