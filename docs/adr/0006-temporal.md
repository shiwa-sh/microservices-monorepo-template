...
- **Local development:** the inner loop uses `temporal server start-dev` (`dep:temporal`) for speed; the
...
- `dep:temporal` (via `infra/local/deps.yaml`) brings up Temporal when a service declares it.
