...
| 4 | Object storage (MinIO, and the production bucket) | Any S3-compatible service | Removes the separate-failure-domain backup problem. Production already points at an external bucket ([ADR-0205](adr/0205-environment-parity.md)), so this row mostly retires the non-prod instance |
